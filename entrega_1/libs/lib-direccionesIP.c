...
{
    char addr_ip_text[ARBITRARY_LENGTH];           // Aquí guardaremos el nombre de host que buscamos. No sabemos a priori cuánto puede ocupar
...
int get_port_info(char *port)
{
    ...
    int error_check;              // Usaremos esta variable para comprobar errores en la llamada a funciones del sistema
    char service[ARBITRARY_LENGTH];             // Longitud arbitraria porque no sabemos cómo de grande puede ser el nombre del servicio
...
int get_host_and_port_info(char* name, char* service) {
    ...
    // res: El puntero a lista enlazada que se declara arriba
    // Devuelve 0 si todo fue bien, en caso de error devuelve un valor diferente
    error_check = getaddrinfo(name, service, &addrinfo, &result);
...
int get_name_and_service_info(char* addr, char* port) {
    ...
    error_check = getnameinfo(addr_info_ret->ai_addr, addr_info_ret->ai_addrlen, addr_ip_text, sizeof(addr_ip_text), service, sizeof(service), 0); // Obtenemos información sobre el host de la dirección IP que recibimos como argumento, junto con el puerto especificado. Ya la tenemos encapsulada de antes en addr_info_ret->ai_addr. addr_info_ret->ai_addrlen es la longitud. El tercer parámetro es el puntero a la cadena de texto donde guardaremos el nombre de host traducido, de longitud sizeof(la cadena). El quinto parámetro es el puntero a la cadena de texto donde guardaremos el nombre del servicio, de longitud sizeof(la cadena). El séptimo es 0 porque no queremos especificar ningún flag en concreto.
    ...
    printf("IPv%s %s:%s: %s (%s)\n", (ai_family == AF_INET6) ? "6" : (ai_family == AF_INET) ? "4" : "?", addr, port, addr_ip_text, service);
