...
int crear_servidor(char *puerto, char *mensaje_enviar)
{
    ...
    while (total_mensajes < 5) // Vamos a contestar en total solo 5 solicitudes, este es un valor arbitrario
    {
        ...
        bytes_enviados = send(socket_conexion, strcat(strcat(ip_cliente, ": "), mensaje_enviar), strlen(strcat(strcat(ip_cliente, ": "), mensaje_enviar)), 0); // Enviamos el mensaje al cliente. El mensaje es la concatenación de la ip en formato texto, ": ", y el mensaje que se pasó como argumento a la función
